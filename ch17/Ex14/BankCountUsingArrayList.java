...
//		System.out.println("결과: 해당 계좌가 존재하지 않습니다. ");
	...
	private static Account findAccount(String ano) { 
		...
		for (int i = 0; i < accounts.size(); i++) {
			if (accounts.get(i)!=null) { //배열 방의 값이 null이 아닐 경우
				//각 객체의 방의 ano를 담는 변수 선언
				String dbAno=accounts.get(i).getAno(); //배열의 각 방에 저장된 객체의 ano를 dbAno 변수에 할당
				if (dbAno.equals(ano)) {
					account=accounts.get(i); //account 참조 변수는 객체에 주소 정보를 담는다
					break;
				}
